...
pub fn receive(device: &soapysdr::Device, channel: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Listening for transmissions... (waiting 15 seconds)");
    let mut all_samples = Vec::new();
    let mut consecutive_timeouts = 0;
    
    for iteration in 0..75 {
        match rx_stream.read(&mut [&mut buf[..]], 200_000) { // 200ms timeout
            Ok(len) => {
                consecutive_timeouts = 0;
                if len > 0 {
                    let power: f32 = buf[..len].iter().map(|s| s.norm_sqr()).sum::<f32>() / len as f32;
                    ...
                    if all_samples.len() > 5_000_000 {
                        println!("Sample buffer full, processing...");
                        break;
                    }
                }
            },
            Err(e) => {
                ...
                if consecutive_timeouts < 5 {
                    println!("Read timeout: {} (attempt {}/5)", e, consecutive_timeouts);
                }
                if consecutive_timeouts >= 10 {
                    println!("Too many consecutive timeouts, stopping");
                    break;
                }
            }
        }
    ...
    println!("Processing {} total samples...", all_samples.len());
    
    let total_power: f32 = all_samples.iter().map(|s| s.norm_sqr()).sum::<f32>() / all_samples.len() as f32;
