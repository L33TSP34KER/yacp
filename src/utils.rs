...
pub fn init_driver_sdr(channel: usize, _num: usize, freq: f64) -> Option<soapysdr::Device> {
    for dev in soapysdr::enumerate("").unwrap() {
        ...
        dev.set_frequency(soapysdr::Direction::Rx, channel, freq, ()).expect("Failed to set RX frequency");
        dev.set_frequency(soapysdr::Direction::Tx, channel, freq, ()).expect("Failed to set TX frequency");
        
        dev.set_sample_rate(Rx, channel, 1_000_000.0).expect("Failed to set RX sample rate");
        dev.set_sample_rate(Tx, channel, 1_000_000.0).expect("Failed to set TX sample rate");
...
pub fn iq_to_text(samples: &[Complex<f32>]) -> String {
    println!("Decoding {} samples", samples.len());
    
    if samples.len() < 100 {
        return String::from("[Too few samples]");
    }
    ...
    if max_power < 0.01 {
        return String::from("[Signal too weak]");
    }
    ...
    let threshold = 0.3;
    let bits: Vec<u8> = power.iter().map(|&p| if p > threshold { 1 } else { 0 }).collect();
    
    println!("Converted to {} bits, looking for sync pattern...", bits.len());
    ...
    for start in 1000..bits.len().saturating_sub(sync_pattern.len() * samples_per_bit) {
        let mut matches = 0;
        for (i, &expected_bit) in sync_pattern.iter().enumerate() {
            ...
            if bit_end <= bit_start { break; }
            
            let ones = bits[bit_start..bit_end].iter().sum::<u8>();
            let actual_bit = if ones > (samples_per_bit as u8 / 2) { 1 } else { 0 };
        ...
        if matches >= sync_pattern.len() - 2 {
            ...
            println!("Sync pattern found at position {}, data starts at {}", start, data_start);
            break;
        }
    }
    ...
    if text.is_empty() {
        String::from("[No valid text decoded]")
    } else {
        text
    }
}
